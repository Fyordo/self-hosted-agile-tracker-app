<template>
  <div class="tracker">
    ...
    <UIInput class="ticket-description" v-model="this.timeEntry.description" />
    <div class="timer">{{ this.getTrackedTimeAsString() }}</div>

    <div v-if="this.timerEnabled">
      <UIButton @click="stopTimeEntry" :title="'Stop'" />
    </div>
    <div v-else="this.timerEnabled">
      <UIButton @click="startTimeEntry" :title="'Start'" />
    </div>
  </div>
</template>
...
export default {
  created() {
    ...
    axiosAgregator
      .sendGet("/api/time-entry/current")
      .then((response) => {
        let responseData = response.data;
        this.trackedTime = {
          hours: 0,
  ...
  methods: {
    stopTimeEntry() {
      axiosAgregator
        .sendPost("/api/time-entry/end/" + this.timeEntry.id)
        .then((response) => {
          this.timerEnabled = false;
          this.trackedTime = {
            hours: 0,
    ...
    startTimeEntry() {
      axiosAgregator
        .sendPost("/api/task/" + this.selectedTaskId + "/start-entry", {
